...
| 010 | Replace the hand-rolled prompt fence with airlock/wrap | P2 | S | airlock v0.1.0 | DONE (merged in PR #225; airlock pinned at v0.1.0, fence.go is now a thin adapter over airlock/wrap; existing internal/ai tests pass unchanged, evasion regression added. Fixed an encoding-evasion hole airlock caught: homoglyph/zero-width/fullwidth fence tags used to survive neutralization. go1.26.5 vulncheck fix landed first in #224) |
| 011 | Flip security score to a threat scale, drop security_reason | P2 | M | -- | SUPERSEDED (folded into 012; its scale-flip + column-drop are Phase 2 there. Standalone execution would rewrite `security.txt` only for 012 to replace it, forcing two rescores -- see 012 "Relationship to 011") |
| 012 | Adopt airlock/screen + airlock/detect as one screening core | P2 | L | airlock v0.1.0 | IN PROGRESS (all 4 phases code-complete in PR #228, green against PG17; branch `feature/012-airlock-screen-detect`. Absorbs 011. Remaining is the operator rescore: run `herald screen-compare`, then null `security_threat` and let the pipeline re-screen. Local config.toml must rename `security_score` -> `max_security_threat` or it fails closed at boot) |
